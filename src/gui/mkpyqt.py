...
if sys.platform.startswith("darwin"):
    ...
    if i > -1:
        PATH = PATH[:i] + "bin"
PYUIC4 = os.path.join(PATH, "pyuic") # e.g. PYUIC4 = "/usr/bin/pyuic"
if sys.platform.startswith("darwin"):
...
PYRCC4 = os.path.join(PATH, "pyrcc")
PYLUPDATE4 = os.path.join(PATH, "pylupdate")
LRELEASE = "lrelease"
...
if not os.access(PYUIC4, os.F_OK):
    msg.append("failed to find pyuic; tried %s" % PYUIC4)
if not os.access(PYRCC4, os.F_OK):
    msg.append("failed to find pyrcc; tried %s" % PYRCC4)
if not os.access(PYLUPDATE4, os.F_OK):
    msg.append("failed to find pylupdate; tried %s" % PYLUPDATE4)
if msg:
    print "\n".join(msg)
